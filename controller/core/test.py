...
# emb = np.load("vector.npy")
# emb2 = np.load("vector2.npy")
# from tensorflow.keras.models import load_model
from jcopml.utils import save_model, load_model

def distance(emb1, emb2):
    return np.sum(np.square(emb1 - emb2))
# np.savetxt("vector.txt",np.array(emb).astype("float32"))
# print(distance(emb, emb2))
        ...
        for i in range(0, detections.shape[2]):
            ...
            if confidence > 0.7:
                ...
                try:
                    ...
                    # proba = preds[j]
                    # name = names[j]
                    # np.save("vector2.npy", np.array(vec).astype("float64"))
                    # print(100-proba * 100)
                    # os._exit(0)
                    # name = names[preds[0]]
